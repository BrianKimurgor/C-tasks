...
{
       0,    32,    32,    32,    34,    34,    36,    37,    39,    41,
      42,    43,    48,    49,    52,    91,    95,    98,   106,   110,
     113,   120,   127,   134,   141
};
#endif
...
{
  "$end", "error", "$undefined", "IF", "THEN", "ELSE", "FI", "condition",
  "WORD", "EOLN", "PIPE", "INFILE", "OUTFILE", "OUTFILE_APPEND", "ERRFILE",
  "ERRFILE_APPEND", "$accept", "input", "lines", "oneline", "eoln", "line",
  "if_block", "cmd", "optargs", "arg", "optredirs", "redir", YY_NULLPTR
};
#endif
...
{
      -2,    -5,     1,    -7,    10,    -7,    -2,    -7,     2,    -7,
       9,     8,    -7,     3,     1,    -7,    -7,    -7,     0,    -2,
      12,    13,    14,    15,    16,    -7,     3,    -7,    -7,    -1,
      -7,    -7,    -7,    -7,    -7,    -7,    -2,    -7,    19,    -7
};
...
{
       3,     0,    16,     8,     0,     2,     4,     7,     0,    10,
       9,     0,    17,    19,    16,     1,     5,     6,     0,     0,
       0,     0,     0,     0,     0,    14,    19,    15,    11,     0,
      20,    21,    22,    23,    24,    18,     0,    13,     0,    12
};

  /* YYPGOTO[NTERM-NUM].  */
static const yytype_int8 yypgoto[] =
{
      -7,    -7,    -6,    -7,    18,    11,    -7,    -7,    17,    -7,
       6,    -7
};
...
{
      16,     1,    11,     1,    36,    37,     2,     3,     2,    12,
      15,     3,    19,    29,    20,    21,    22,    23,    24,    18,
      30,    31,    32,    33,    34,    39,    17,     0,     0,    28,
      38,    27,    35
};
...
{
       6,     3,     7,     3,     5,     6,     8,     9,     8,     8,
       0,     9,     4,    19,    11,    12,    13,    14,    15,    10,
       8,     8,     8,     8,     8,     6,     8,    -1,    -1,    18,
      36,    14,    26
};
...
{
       0,     3,     8,     9,    17,    18,    19,    20,    21,    22,
      23,     7,     8,    24,    25,     0,    18,    20,    10,     4,
      11,    12,    13,    14,    15,    26,    27,    24,    21,    18,
       8,     8,     8,     8,     8,    26,     5,     6,    18,     6
};
...
#line 52 "parser.y"
         ...
         newcmd->command = (yyvsp[-2].string);   // Set the command name to the WORD token
         
         // Create a new args structure to include the command name
         struct args *cmd_arg = (struct args *) MallocZ(sizeof(struct args));
         cmd_arg->arg = (yyvsp[-2].string);       // Assign the command name as the first argument
         cmd_arg->next = (yyvsp[-1].pargs);      // Link the existing arguments to this new argument
         
         newcmd->args = cmd_arg;  // Set arguments with command name as the first entry
         newcmd->redirs = (yyvsp[0].predir);     // Set the redirections
         ...
         struct redirs *current_redir = (yyvsp[0].predir); // Use the parsed redirs list
         ...
         (yyval.pcmd) = newcmd;            // Return the new command structure
     }
...
#line 91 "parser.y"
                      {
            (yyvsp[-1].pargs)->next = (yyvsp[0].pargs);       // Link new argument to the argument list
            (yyval.pargs) = (yyvsp[-1].pargs);
        }
...
#line 98 "parser.y"
        ...
        newarg->arg = (yyvsp[0].string);       // Assign the WORD token as the argument
        newarg->next = NULL;    // Set the next pointer to NULL
        (yyval.pargs) = newarg;            // Return the new argument structure
    }
...
#line 106 "parser.y"
                            {
              (yyvsp[-1].predir)->next = (yyvsp[0].predir);    // Link new redirection to the redirection list
              (yyval.predir) = (yyvsp[-1].predir);
          }
...
#line 113 "parser.y"
       ...
       newredir->filename = (yyvsp[0].string);  // Capture the filename for input redirection
       newredir->next = NULL;
       (yyval.predir) = newredir;            // Return the new redirection structure
   }
...
#line 120 "parser.y"
       ...
       newredir->filename = (yyvsp[0].string);  // Capture the filename for output redirection
       newredir->next = NULL;
       (yyval.predir) = newredir;            // Return the new redirection structure
   }
...
#line 127 "parser.y"
       ...
       newredir->filename = (yyvsp[0].string);  // Capture the filename for append output redirection
       newredir->next = NULL;
       (yyval.predir) = newredir;            // Return the new redirection structure
   }
...
#line 134 "parser.y"
       ...
       newredir->filename = (yyvsp[0].string);  // Capture the filename for error redirection
       newredir->next = NULL;
       (yyval.predir) = newredir;            // Return the new redirection structure
   }
...
#line 141 "parser.y"
       ...
       newredir->filename = (yyvsp[0].string);  // Capture the filename for append error redirection
       newredir->next = NULL;
       (yyval.predir) = newredir;            // Return the new redirection structure
   }
